...
class DotProductComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output(name=opts['c_name'], shape=(m), units=opts['c_units'])

        row_idxs = np.repeat(np.arange(m), vec_size)
        col_idxs = np.arange(vec_size * m)
        self.declare_partials(of=opts['c_name'],
                              wrt=opts['a_name'],
                              rows=row_idxs,
                              cols=col_idxs)
        self.declare_partials(of=opts['c_name'],
    ...
    def compute(self, inputs, outputs):
        ...
        outputs[opts['c_name']] = np.einsum('ni,ni->i', a, b)
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[opts['c_name'], opts['a_name']] = b.flatten()
        partials[opts['c_name'], opts['b_name']] = a.flatten()
